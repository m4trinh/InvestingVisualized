...
const GICVisualized = () => {
    ...
    const [gicAction, setGicAction]: [GICActionModel, Function] = useState(getLocalStorage(SessionState.infoGIC, defaultGICAction));
    ...
    const renderMaturitySummary = () => {
        if (gicGraphPoints && gicGraphPoints.length > 0) {
            ...
            return (
            <p className="textAlignCenter">At {interest}% interest, after {maturity} months, your principal of ${principal} has grown to ${gicGraphPoints[lastElementIndex].amount}</p>
            )
        }
    }
