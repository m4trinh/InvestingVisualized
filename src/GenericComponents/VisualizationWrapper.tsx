...
const VisualizationWrapper = (props: Props) => {
    ...
    if (show) {
        return ( 
            <>
            {component}
            <button className="btn btn-danger" onClick={() => toggleShow(!show)}>Close Visualization</button>
            </>
        );
    } else {
        return <button className="btn btn-primary" onClick={() => toggleShow(!show)}>Open Visualization</button>
    }
}
