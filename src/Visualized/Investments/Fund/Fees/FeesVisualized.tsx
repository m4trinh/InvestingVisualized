...
const FeesVisualized = () => {
	...
	const [feeAction, setFeeAction]: [FeeActionModel, Function] = useState(
		getLocalStorage(SessionState.infoFees, defaultFeeAction)
	);
	...
	const updateGraph = () => {
		...
		for (let year = 1; year <= years; year++) {
			const principalAmount = ~~(
				tempPoints[year - 1].principalAmount *
				(1 + interestPercent)
			);
			const feeAmount = ~~(
				tempPoints[year - 1].feeAmount *
				(1 + interestWithFeePercent)
			);
	...
	const renderLossSummary = () => {
		if (feeGraphPoints && feeGraphPoints.length > 0) {
			...
			const totalLoss =
				feeGraphPoints[lastElementIndex].principalAmount -
				feeGraphPoints[lastElementIndex].feeAmount;
			const { fee, years } = feeAction;
			return (
				<p className={styles.lossSummary}>
					${totalLoss} would be lost due to the {fee}% fee over a {years} period.
				</p>
			);
		}
	};
	...
	const renderTooltip = ({ active, payload, label }) => {
		if (active) {
			return (
				<div>
					<h3>{`Year ${label}`}</h3>
					<p>{`So far, you would've lost $${payload[0].value -
						payload[1].value} to fees!`}</p>
				</div>
			);
		}
	};
